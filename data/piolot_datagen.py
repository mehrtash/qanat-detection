...
if __name__ == '__main__':
    labels = sorted(glob.glob(patch_folder + '/*_label.nrrd'))
    multiplier = 100
    chunk_size = 8
    shape = 250
    for label_path in labels:
        ...
        np.save(os.path.join(npy_folder, os.path.basename(image_path).replace('nrrd', 'npy')),
                np.expand_dims(nda, -1) / max_val)
        np.save(os.path.join(npy_folder, os.path.basename(label_path).replace('nrrd', 'npy')),
